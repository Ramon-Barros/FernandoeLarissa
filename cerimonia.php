<?php include_once("header.php");?>


<hr class="mt-2 mb-5">
<!-- Page Content -->
<body  style =" background: url('img/moldura12.png') no-repeat center center fixed;
    -webkit-background-size: cover;
    -moz-background-size: cover;
    background-size: cover;
    -o-background-size: cover;"> 
    

<div class="container">
  <div class="card border-0 shadow my-5"style="background: rgba(255,255,255, 0.3); " >
    <div class="card-body p-5 " id="letra-cerimonias">
      <h1 class="font-weight-light">Olá! queridos amigos e familiares!</h1>
      <p class="lead">É com grande alegria que anunciamos nosso casamento!<br>
              Para vocês que fazem parte de nossas vidas, um convite para nos verem solteiros pela última vez, casados pela única e felizes para sempre!<p class="lead">
              <p class="lead">
            Esperamos ver o rostinho de cada um de vocês no nosso casamento para compartilhar e completar a nossa felicidade!!!</p>
      <p class="lead text-center">Grande beijo! </p>
    </div>
      <div style="height: 100px"></div>

        <div id="letra-cerimonias2">
        <ul>
          <li class="row">
            <figure class="col-sm-6 col-md-3">
              <img class="card-img" src="img/svp.png" alt="Imagem do card">
            </figure>

            <div class="col-sm-6 col-md-7">
                <h3>Igreja São Vicente de Paulo às 19h</h3>
                <p>Endereço: Av. Getúlio Vargas, s/n - Monte Castelo, São Luís - MA, 65030-005 Ref: entre o curso Wellington e 24º BC </p>
              </div>
          </li>
          <div style="height: 50px"></div>

          <li class="row">
            <figure class="col-sm-12 col-md-3">
              <img class="card-img" src="img/class.png" alt="Imagem do card">
            </figure>

            <div class="col-sm-12 col-md-7">
                <h3> Class Home às 21:30min</h3>
                <p>Endereço: Av. dos Holandeses, S/N - Lote 16 - Calhau, São Luís - MA, 
                65071-380 próximo ao retorno do shopping do Automóvel.</p>
              </div>
          </li>

          
        </ul>
          </div>  

    
  </div>
</div>

</body>
                
<?php include_once("footer.php");?>